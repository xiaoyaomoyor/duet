...
**v0.7.0 · R2 舞台工作区**：新建项目时选择“音乐评测”“图片评测”或“通用评测”，即可编辑并保存真实内容。左侧组织内容、中间预览成稿、右侧修改属性；支持墨色／纸白、独听、阅读报告与 16:9 剧场演示。

![R2 真实项目试听舞台](docs/screenshots/r2-stage-ink.png)

旧项目点击“另存舞台副本”体验新界面，原稿保留。PNG 可导出当前场景或完整报告，HTML 输出可离线阅读的报告，`.duet` 保留可编辑数据。实现范围、实测画面见 [R2 验收记录](docs/05-R2舞台工作区与验收.md)。截图内容为验收示例，不代表平台性能结论。
