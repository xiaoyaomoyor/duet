...
**v0.12.0 · W2 音乐创作闭环**：可以从空白舞台上传自己的音乐与封面，填写歌词、观察与评分，组合模块和场景，保存后进入演示，再回到原编辑位置。新版、旧版与样板间都位于对奏现有主界面中；新建舞台默认新版。

![W2 新版音乐编辑器](docs/screenshots/w2-editor.png)

左侧按实际场景组织创作；右侧集中处理当前模块、作品库与外观，复杂内容使用统一的扩展编辑窗口。模块可复制、排序、移动和隐藏；场景默认独立复制，明确的「引用已有内容」才共享内容。并置试听与通用组合已接入真实工程，使用步骤和截图见 [W2 验收记录](docs/11-W2音乐创作闭环验收.md)。

旧舞台按原工作区打开；点击顶栏工作区名称可在同一份舞台内手动切换。切换先保存，保留 ID、内容、素材、外观和演示步骤；多对象、多测试题、多作品等旧版无法完整编辑的舞台会显示具体限制，保留全部数据。工程 schema 13 新增场景版式，保留历史恢复快照。工作区规则见 [W1 验收记录](docs/10-W1工作区入口与迁移验收.md)。

新版已有 2—6 个对象、多作品、步骤演示、四套精选风格与个人预设。PNG 可导出当前步骤、完整场景或阅读报告；HTML 支持离线作品切换与步骤演示，超限可选择静态阅读页；`.duet` 保留可编辑数据，`.duetpack` 携带完整本地素材。添加对象的入口是「作品库 → 对比对象」，或左侧「对象资料」；外观入口在顶栏「外观与版式」，`.duetstyle` 仅分享样式。交付方式及 Safari／性能验收边界见 [R5 验收记录](docs/08-R5外观预设与交付验收.md)。

顶栏「样板间」及 `#/showcase` 保留视觉示例，其中的临时修改尚不写入舞台。原「组件规范」移至「设置 → 关于 → 开发与设计 → 设计规范」，用于开发时检查控件一致性。W3 将继续落实命题、观察、数据与结论的专用构图，并支持从样板创建可编辑舞台，详见 [新版工作区产品化施工方案](docs/09-新版工作区产品化施工方案.md)。
...
多样本、按键演示、六方对比、个人预设及素材包已按 R0—R5 实施。新版工作区已完成 W1 入口整合与 W2 音乐创作闭环，样板版式及整体验收继续按 W3—W4 推进。

## 项目文档

| 文档 | 内容 |
| --- | --- |
| [`docs/09-新版工作区产品化施工方案.md`](docs/09-新版工作区产品化施工方案.md) | **当前施工依据**：工作区统一、音乐创作闭环、样板版式与可用性验收 |
| [`docs/10-W1工作区入口与迁移验收.md`](docs/10-W1工作区入口与迁移验收.md) | v0.11.0 入口、切换、迁移、截图与验证结果 |
| [`docs/11-W2音乐创作闭环验收.md`](docs/11-W2音乐创作闭环验收.md) | v0.12.0 音乐创作、场景与模块操作、统一编辑窗口、截图与验证结果 |
...
- [x] **W1 工作区入口与迁移**：新建默认新版，同一舞台切换，历史工程恢复，样板间融入主界面，设计规范移至设置
- [x] **W2 音乐创作闭环**：真实素材创作与演示、场景独立复制和引用、模块完整操作、统一属性栏与扩展编辑
- [ ] **W3—W4 新版工作区产品化**：样板版式正式落地、可用性收口
...
- **浏览器验证范围分阶段记录**：W2 在 Windows Edge 完成回归；R5 的 Firefox 与 Windows WebKit 结果见验收记录，Safari 仍需 macOS／iOS 实机验证。
