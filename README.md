...
**v0.11.0 · W1 工作区入口与迁移**：新建音乐、图片、视频、通用或空白舞台，默认进入新版工作区。新版、旧版与样板间都位于对奏现有主界面中。首页采用统一的中性色与编辑式排版，小屏自动收起舞台列表。

![W1 舞台首页](docs/screenshots/w1-home.png)

旧舞台按原工作区打开；点击顶栏工作区名称可在同一份舞台内手动切换。切换先保存，保留 ID、内容、素材、外观和演示步骤；多对象、多测试题、多作品等旧版无法完整编辑的舞台会显示具体限制，保留全部数据。工程升级至 schema 12，并保留历史恢复快照。使用方式与迁移验证见 [W1 验收记录](docs/10-W1工作区入口与迁移验收.md)。

新版已有 2—6 个对象、多作品、步骤演示、四套精选风格与个人预设。PNG 可导出当前步骤、完整场景或阅读报告；HTML 支持离线作品切换与步骤演示，超限可选择静态阅读页；`.duet` 保留可编辑数据，`.duetpack` 携带完整本地素材。添加对象的入口是「作品与流程 → 对比对象」；外观入口在顶栏「外观与版式」，`.duetstyle` 仅分享样式。交付方式及 Safari／性能验收边界见 [R5 验收记录](docs/08-R5外观预设与交付验收.md)。

顶栏「样板间」及 `#/showcase` 保留视觉示例，其中的临时修改尚不写入舞台。原「组件规范」移至「设置 → 关于 → 开发与设计 → 设计规范」，用于开发时检查控件一致性。W2 将补齐音乐创作闭环，W3 将把样板构图落实为正式可编辑版式，详见 [新版工作区产品化施工方案](docs/09-新版工作区产品化施工方案.md)。
...
多样本、按键演示、六方对比、个人预设及素材包已按 R0—R5 实施。新版工作区的入口整合已完成 W1，编辑与样板版式继续按 W2—W4 推进。
...
- [ ] **W2—W4 新版工作区产品化**：音乐创作闭环、样板版式正式落地、可用性收口
...
- **浏览器验证范围分阶段记录**：W1 在 Windows Edge 完成回归；R5 的 Firefox 与 Windows WebKit 结果见验收记录，Safari 仍需 macOS／iOS 实机验证。
